...
export async function request(endpoint, options = {}) {
  ...
  const token = typeof window !== 'undefined' ? localStorage.getItem('odyssey_token') : null;
...
export const apiClient = {
  ...
  post: (endpoint, body, params) => request(`${endpoint}${buildQuery(params)}`, { method: 'POST', body: body ? JSON.stringify(body) : undefined }),
  put: (endpoint, body, params) => request(`${endpoint}${buildQuery(params)}`, { method: 'PUT', body: body ? JSON.stringify(body) : undefined }),
  delete: (endpoint, params) => request(`${endpoint}${buildQuery(params)}`, { method: 'DELETE' }),
};
